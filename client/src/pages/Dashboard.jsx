...
import { FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloudIcon, XIcon, XSquareIcon } from "lucide-react";
 ...
 function DashBoard() {
  ...
  prev.map(resume =>
    resume._id === editresumeId ? { ...resume, title } : resume
      ...
      toast.error(error?.response?.data?.message || "Error updating resume title");
    }
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 ">
        ...
        <div className="flex gap-4">
          <button
            ...
            justify-center rounded-1g gap-2 text-slate-600 border border-dashed
          ...
          >
            <UploadCloudIcon
              className="size-11 transition-all duration-300 p-2.5
            bg-linear-to-br from-purple-300 to-purple-500 text-white rounded-full"
            />
            <p
...
<div className="grid grid-cols-2 sm:flex flex-wrap gap-4 mb-8">
...
{/* ================= PUBLIC RESUMES ================= */}
<h2 className="text-lg font-semibold mb-3">Public Resumes</h2>

<div className="grid grid-cols-2 sm:flex flex-wrap gap-4">
  {publicResumes.map((resume, index) => {
    const baseColor = colors[index % colors.length];
    return (
      <button
        key={resume._id}
      ...
      >
        ...
        <span className="absolute bottom-1 text-xs bg-green-500 text-white px-2 py-0.5 rounded">
          Public
        </span>
      </button>
    );
          ...
          {showCreateResume && (
            <form onSubmit={createResume} onClick={()=>setShowCreateResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              <div onClick={(e) => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6"> 
                ...
                <XSquareIcon className="absolute top-4 right-4 size-6 text-slate-400 cursor-pointer transition-colors hover:text-slate-600" onClick={() => {setShowCreateResume(false);setTitle("")}} />
              </div>
            </form>
          ...
          {showUploadResume && (
            <form onSubmit={uploadResume} onClick={()=>setShowUploadResume(false)} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              <div onClick={(e) => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6"> 
                ...
                <XSquareIcon className="absolute top-4 right-4 size-6 text-slate-400 cursor-pointer transition-colors hover:text-slate-600" onClick={() => {setShowUploadResume(false);setTitle("")}} />
              </div>
            </form>
          )}
          ...
          {editresumeId && (
            <form onSubmit={editTitle} onClick={()=>setEditResumeId('')} className="fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center">
              <div onClick={(e) => e.stopPropagation()} className="relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6"> 
                ...
                <XSquareIcon className="absolute top-4 right-4 size-6 text-slate-400 cursor-pointer transition-colors hover:text-slate-600" onClick={() => {setEditResumeId('');setTitle("")}} />
              </div>
            </form>
